...
<html>
<head>
    <title><fmt:message key="title.page.payment.confirmation"/></title>
    <meta charset="utf-8">
        ...
        <div class="col-sm-9">
          <div class="well">
              <c:if test="${not empty paymentNotEligibleToSend}">
                  <p class="text-danger">${paymentNotEligibleToSend} ${paymentIdToSend}</p>
              </c:if>
              ...
              <p class="text-danger">${paymentIncorrectConfirmationCode}</p>
              <p class="text-danger">${systemFailureDuringPaymentProcessing}</p>
              <form action="controller" method="post">
                  <h4><fmt:message key="label.payment.confirm.code"/></h4>
                  <div class="form-group">
                      <label><fmt:message key="label.payment.confirmation"/>:</label>   
                      <input name="userPaymentConfirmationCode" type="text"
                             placeholder="<fmt:message key="label.payment.confirm.code"/>" autofocus required>
                  </div>
                  <input type="hidden" name="page" value="${param.page}">
                  <input type="hidden" name="paymentIdToSend" value="${param.paymentIdToSend}">
                  <button type="submit" value="send_payment_confirmation" name="action" class="btn btn-primary">
                      <fmt:message key="label.payment.send"/>
                  </button>
              </form>
          </div>
      </div>
